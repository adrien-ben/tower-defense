...
public class TowerDefense extends GameApplication {
    ...
    protected void render(Graphics2D graphics2D) {
        ...
        int generatorSize = (int)(level.getCellSize()*PIXELS_PER_UNIT);
        world.getEntities(EnemyGenerator.class).stream().forEach(generator -> renderEntity(graphics2D, generator, generatorSize, true));
        graphics2D.setColor(Color.BLUE);
        int enemySize = (int)(ENEMY_SIZE*PIXELS_PER_UNIT);
        world.getEntities(Enemy.class).stream().forEach(enemy -> renderEntity(graphics2D, enemy, enemySize, false));
        graphics2D.setColor(Color.GREEN);
        int turretSize = (int)(TURRET_SIZE*PIXELS_PER_UNIT);
        world.getEntities(Turret.class).stream().forEach(turret -> renderEntity(graphics2D, turret, turretSize, false));
        graphics2D.setColor(Color.RED);
        int bulletSize = (int)(BULLET_SIZE*PIXELS_PER_UNIT);
        world.getEntities(Bullet.class).stream().forEach(bullet -> renderEntity(graphics2D, bullet, bulletSize, true));
    }
    ...
    private void renderEntity(Graphics2D graphics2D, Entity entity, int size, boolean fill) {
        Vector2 position = entity.getPosition();
        if(fill) {
            graphics2D.fillOval((int)(position.getX()*PIXELS_PER_UNIT) - size/2, (int)(position.getY()*PIXELS_PER_UNIT) - size/2, size, size);
        } else {
